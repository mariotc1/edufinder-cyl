...
const STORAGE_KEY = 'edufinder_search_history';
const MAX_HISTORY_ITEMS = 8;

// HOOK PERSONALIZADO: GESTIÓN DE HISTORIAL DE BÚSQUEDAS
...
export function useSearchHistory() {
    ...
    const syncWithBackend = useCallback(async (localItems: SearchHistoryItem[]): Promise<SearchHistoryItem[]> => {
        try {
            // Enviar términos locales al backend para merge
            const terms = localItems.map(item => item.search_term);
            const response = await api.post('/search-history/sync', { items: terms });
            ...
            console.error('Error syncing search history with backend:', error);
            return localItems;
        }
    }, []);

    // Cargar desde el backend (solo para usuarios logueados)
    const loadFromBackend = useCallback(async (): Promise<SearchHistoryItem[]> => {
        try {
    ...
    const addToHistory = useCallback(async (searchTerm: string) => {
        ...
        if (user) {
            try {
                const response = await api.post('/search-history', { search_term: term });
