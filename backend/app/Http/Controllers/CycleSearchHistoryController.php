...
use App\Models\CycleSearchHistory;
use Illuminate\Http\Request;

/**
 * CONTROLADOR DE HISTORIAL DE BÚSQUEDAS DE CICLOS FP
 * Gestiona el historial de búsquedas de ciclos por usuario
 */
class CycleSearchHistoryController extends Controller
{
    ...
    {
        $history = $request->user()->cycleSearchHistories()
    ...
    public function store(Request $request)
    {
        ...
        $existing = $request->user()->cycleSearchHistories()
        ...
        $item = $request->user()->cycleSearchHistories()->create([
    ...
    {
        $deleted = CycleSearchHistory::where('user_id', $request->user()->id)
            ->where('id', $id)
            ->delete();
    ...
    {
        $request->user()->cycleSearchHistories()->delete();
    ...
    public function sync(Request $request)
    {
        ...
        foreach ($validated['items'] as $searchTerm) {
            ...
            CycleSearchHistory::updateOrCreate(
        ...
        $history = CycleSearchHistory::where('user_id', $userId)
            ->orderBy('updated_at', 'desc')
            ->limit(self::MAX_HISTORY_ITEMS)
            ->get(['id', 'search_term', 'updated_at']);
    ...
    {
        $count = CycleSearchHistory::where('user_id', $userId)->count();

        if ($count > self::MAX_HISTORY_ITEMS) {
            $toDelete = $count - self::MAX_HISTORY_ITEMS;

            CycleSearchHistory::where('user_id', $userId)
